...
int parseJSON(){
    ...
    if(mysql_real_connect(&sql,"127.0.0.1","root","","aen",0,NULL,0))
    {
          ...
          sprintf(marequete, "UPDATE meteo SET weather = '%s', tempnow = '%s', pressure = '%s', humidity ='%s', tempsmin = '%s', tempsmax = '%s', visibility = '%s', windspeed = '%s', winddegree = '%s', sunrise = '%s', sunset= '%s' WHERE iDMeteo = 109;"
